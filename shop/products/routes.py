...
@app.route('/contacts')
def contacts():
    return render_template('products/contacts.html')
...
def get_brand(id):
    ...
    brand = Addproduct.query.filter_by(
        brand=get_brand).paginate(page=page, per_page=8)
...
def addbrand():
    if request.method == "POST":
        ...
        flash(f'Коллекция {getbrand} была добавлена в базу данных', 'success')
        db.session.commit()
        return redirect(url_for('addbrand'))
    return render_template('products/addbrand.html', title='Добавление коллекции', brands='brands')
...
def updatebrand(id):
    ...
    if request.method == "POST":
        ...
        flash(f'Коллекция {updatebrand.name} изменена на {brand}', 'success')
        db.session.commit()
        return redirect(url_for('brands'))
    brand = updatebrand.name
    return render_template('products/addbrand.html', title='Обновить коллекцию', brands='brands', updatebrand=updatebrand)
...
def deletebrand(id):
    ...
    if request.method == "POST":
        ...
        flash(f"Коллекция {brand.name} был удален из базы данных", "success")
        db.session.commit()
        return redirect(url_for('admin'))
    flash(f" {brand.name} не может быть удален из базы данных", "warning")
    return redirect(url_for('admin'))
...
def addcat():
    if request.method == "POST":
        ...
        flash(
            f'Категория {getcat} была добавлена в вашу базу данных', 'success')
        db.session.commit()
...
def updatecat(id):
    ...
    if request.method == "POST":
        ...
        flash(
            f'Категория{updatecat.name} была изменена на {category}', 'success')
        db.session.commit()
        return redirect(url_for('categories'))
    category = updatecat.name
    return render_template('products/addbrand.html', title='Обновить категорию', updatecat=updatecat)
...
def deletecat(id):
    ...
    if request.method == "POST":
        ...
        flash(f"Категория {category.name} была удалена базы данных", "success")
        db.session.commit()
        return redirect(url_for('admin'))
    flash(
        f"Категория {category.name} не может быть удалена с базы данных", "warning")
    return redirect(url_for('admin'))
...
def updateproduct(id):
    ...
    if request.method == "POST":
        ...
        flash('Товар был обновлен', 'success')
        db.session.commit()
        return redirect(url_for('admin'))
    ...
    return render_template('products/addproduct.html', form=form, title='Обновить товар', getproduct=product, brands=brands, categories=categories)
...
def deleteproduct(id):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(f'Товар {product.name} был удален', 'success')
        return redirect(url_for('adim'))
    flash(f'Невозможно удалить товар', 'warning')
    return redirect(url_for('admin'))
